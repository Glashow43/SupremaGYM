...
/**
 * Retourne tous les exercices : intégrés + custom (localStorage).
 * Les exercices custom ont { custom: true }.
 */
function getAllExercises() {
  const custom = (ld('customExercises', []) || []).map(e => ({ ...e, custom: true }));
  return [...BUILTIN_EXERCISES, ...custom];
}

// ── Programme par défaut (POWER RUSH PANA) ───────────────
/**
 * Retourne l'objet programme injecté à la première connexion.
 * ID fixé à 1001 pour éviter les doublons.
 */
